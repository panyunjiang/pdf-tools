...
export default function SplitClient() {
  ...
  const handleSplit = async () => {
    ...
    try {
      ...
      alert('Split failed. Please ensure the file is a valid PDF.');
    } finally {
      setProcessing(false);
    }
  };
  ...
  return (
    <ToolPageLayout
      icon={Scissors}
      title="Split PDF"
      description="Extract specific pages from a PDF by page range. Supports single, multiple, and range selection."
    >
      <div className="space-y-6">
        ...
        {totalPages > 0 && (
          <div className="space-y-3">
            <p className="text-sm text-gray-600">Total {totalPages} pages</p>
            <div>
              ...
              />
              <p className="text-xs text-gray-500 mt-1">Separate multiple ranges with commas, e.g.: 1-3, 5, 7-10</p>
            </div>
          </div>
        )}
